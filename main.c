...
main (int argc, char * argv [])
{
    ...
    GOptionEntry entries [] = {
        {"host", 'h', 0, G_OPTION_ARG_STRING, &ad->host, "Host", "HOST"},
        {"username", 'u', 0, G_OPTION_ARG_STRING, &ad->uname, "Username", "STR"},
        {"password", 'p', 0, G_OPTION_ARG_STRING, &ad->passw, "Password", "STR"},
        {NULL}};
    GError * error = NULL;
...
smenu_view (gpointer data)
{
    ...
    GtkWidget * items [3] = {gtk_menu_item_new_with_label("rename"),
...
// Rename a selected item in the connection list
static void
item_rename_cb (GtkWidget * wdg, gpointer data)
{
    ...
    dialog = gtk_dialog_new_with_buttons("Rename", win, GTK_DIALOG_MODAL, "OK",
...
item_remove_cb (GtkWidget * wdg, gpointer data)
{
    ...
    win = GTK_WINDOW(wd->win);
    dialog = gtk_message_dialog_new(win, GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
        GTK_BUTTONS_YES_NO, "Are you sure you want to delete this?");
    ...
    case GTK_RESPONSE_YES:
        ...
        // Create the rows references, to modify a tree model safely
...
// Show a selected connection info
static void
item_info_cb (GtkWidget * wdg, gpointer data)
{
    ...
    dialog = gtk_dialog_new_with_buttons(
        "Info", win, GTK_DIALOG_MODAL, NULL, NULL);
...
application_quit (GtkWidget * wdg, gpointer data)
{
    ...
    // FIXME: Use the gtk_application_remove_window()?
...
connect (MYSQL * con, const struct con_info * ci)
{
    ...
    if (!mysql_real_connect(
            con, ci->host, ci->uname, ci->passw, NULL, 0, NULL, 0)) {
        g_print("failed!\n");
        con_assert(con);
...
win_dbs (GtkApplication * app, MYSQL * con)
{
    ...
    g_signal_connect_swapped(
        btn_disconnect, "clicked", G_CALLBACK(gtk_widget_destroy), win);
...
win_table (GtkApplication * app, MYSQL * con, const gchar * tb_name)
{
    ...
    title = g_strdup_printf(
        "%s -> %s -> %s -> %s", con->host, con->user, con->db, tb_name);
